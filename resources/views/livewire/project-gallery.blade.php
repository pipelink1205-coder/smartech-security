    ...
    <div class="container">
        <x-section-header
            ...
            :subtitle="$featuredOnly
                ? 'Trabajos destacados en Medellín y el Valle de Aburrá.'
                : 'Filtra por servicio y abre cada trabajo para ver las fotos de evidencia.'"
        >
            <x-slot:heading>
        ...
        {{-- Móvil: botones por servicio --}}
        <div class="project-mobile-only">
            @if($showMobileHub)
                <p class="project-mobile-hub-hint">Elige un servicio para ver las fotos.</p>
                ...
                <div class="project-service-detail">
                    ...
                    <header class="project-service-group-head">
                        ...
                        <div class="project-service-group-copy">
                            ...
                            <span class="project-service-detail-meta">
                        ...
                        <div class="project-portfolio-grid" role="list">
                            @foreach($projects as $project)
                                <button
                                    ...
                                    wire:key="project-card-m-{{ $project->id }}"
        ...
        {{-- PC: filtros + grilla de proyectos (como antes) --}}
        <div class="project-desktop-only">
                ...
                <div class="project-service-filters" role="tablist" aria-label="Filtrar por servicio">
                    ...
                    @foreach($services as $service)
                        <button
                            ...
                            class="project-service-pill {{ $selectedService === $service->slug ? 'is-active' : '' }}"
                            aria-selected="{{ $selectedService === $service->slug ? 'true' : 'false' }}"
                            title="{{ $service->name }}"
                            aria-label="{{ $service->name }} ({{ $service->projects_count }})"
                        >
                            <span class="project-service-pill-icon" aria-hidden="true">
                                <x-service-icon :icon="$service->icon" :name="$service->name" size="sm" />
                            </span>
                        ...
                        <div class="project-portfolio-grid" role="list">
                            @foreach($block['items'] as $project)
                                <button
                                    ...
                                    role="listitem"
                                    wire:click="openProject({{ $project->id }})"
                                    wire:key="project-card-d-{{ $project->id }}"
                                    class="project-card project-card-selectable has-image"
                                    aria-label="Ver fotos de {{ $project->title }}"
                                >
                                    <img src="{{ $project->image_url }}" alt="{{ $project->title }}" loading="lazy" />
                                    <div class="project-overlay project-overlay--compact">
                                        <span class="project-service-plain">{{ $project->service_name }}</span>
                                        <h3>{{ $project->title }}</h3>
                                        @if($project->location)
                                            <p class="project-card-location">{{ $project->location }}</p>
                                        @endif
        ...
        @if($featuredOnly)
            <div class="projects-cta-row">
                <a href="{{ route('proyectos') }}" class="btn btn-ghost project-desktop-only-inline">Ver portafolio completo</a>
