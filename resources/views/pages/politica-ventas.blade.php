<x-app-layout :title="'Política de Ventas – Smart Tech Security'">
    <article class="legal container">
        ...
        <section class="legal-block">
            <h2>9. Contacto</h2>
            <p>Para dudas sobre el proceso de compra escríbanos a <a href="mailto:{{ config('contact.email') }}">{{ config('contact.email') }}</a> o por WhatsApp al <x-whatsapp-link from="legal" target="_blank">{{ config('contact.whatsapp_formatted') }}</x-whatsapp-link>.</p>
        </section>
    </article>
</x-app-layout>
